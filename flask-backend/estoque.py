...
class Estoque:
  ...
  def imprimir(self):
    print('Estoque ' + self.descricao, self.data, self.hora)
    for item in self.estoque_itens:
      total_unidades = item.pacotes * item.insumo.unidades_pacote + item.unidades
      if self.descricao == 'Diferencial':
        print(item.unidades, item.insumo.descricao)
      else:
        print(item.pacotes, item.unidades, total_unidades, item.insumo.descricao)
